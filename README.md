...
- **Versão atual:** `26.4.0`, com versionamento CalVer.
...
- Transcript mais parecido com Codex/Claude Code, com passos curtos, `Ran ...`, `Worked for ...`, `Updated Plan` e interação de terminal em segundo plano.
- Aviso automático quando houver versão nova, sugerindo `nexus update` para atualizar se o usuário quiser.
...
Na linha `26.4.0`, o NEXUS AGENT suporta **MCP (Model Context Protocol)** via `stdio`.

### MCP na versão 26.4.0
...
A versão `26.4.0` mantém **Notebook integration (Jupyter)**, **automação remota por bots** e **agentes múltiplos em paralelo**, agora com runtime offline/hybrid, aprovação manual de plano e endurecimento forte de segurança.

### O que entra na 26.4.0
